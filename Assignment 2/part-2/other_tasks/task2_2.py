...
def mm_action(state_tup, V, all_action_results):
    ...
    for action, responses in all_action_results.items():
        ...
        for (p, s1, _, got_hurt) in final_responses:
            ...
            if action == 'STAY':
                value += ( p * (0 + STATE_REWARD[s1] + GAMMA * (V[s1])))
            else:
...
# value_iteration((POS_C, 2, 0, MM_DORMANT, 100), True)
value_iteration((POS_E, 2, 2, MM_DORMANT, 25), True)

# value_iteration((POS_E, 2, 2, MM_READY, 100), True)
# value_iteration((POS_W,0,0,MM_DORMANT,100), True)

# V(t, s) = (step + gamma*(V(t-1, s1)))
